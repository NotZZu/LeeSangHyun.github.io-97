...
const EditableSection = {
  ...
  template: `
    <div v-if="!isEditing">
      <p v-if="multiline" :class="['whitespace-pre-wrap', $props.class]">{{ modelValue }}</p>
      <span v-else :class="$props.class">{{ modelValue }}</span>
    </div>
    <textarea
      v-else-if="multiline"
      :value="modelValue"
      @input="$emit('update:modelValue', $event.target.value)"
      :class="['min-h-[100px] resize-y rounded-md border border-input bg-background px-3 py-2 text-sm', $props.class]"
    />
    <input
      v-else
      type="text"
      :value="modelValue"
      @input="$emit('update:modelValue', $event.target.value)"
      :class="['rounded-md border border-input bg-background px-3 py-2 text-sm', $props.class]"
    />
  `
}
